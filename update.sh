...
function main() {
    ...
    if [[ "$force" == "true" || $panel_update == 1 || $manager_update == 1 ]]; then
        # تغییر آدرس به ریپوی خودتان:
        bash <(curl -sSL https://raw.githubusercontent.com/MN-HACKER/hiddify-config/main/common/download.sh) "$package_mode" "$force" "--no-gui" "--no-log"
    else
